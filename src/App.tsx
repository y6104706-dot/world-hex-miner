...
function App() {
  ...
  const applyHexSelection = (map: maplibregl.Map, h3Index: string, zoneType: ZoneType) => {
    setSelectedHex({ h3Index, zoneType })
    setSelectedOwned(ownedHexesRef.current.has(h3Index))
    setMineMessage(null)
  ...
  const miningRules: Record<ZoneType, MiningRule> = {
    ...
    PRISON: {
      minSpeedKmh: null,
      description: 'Prison or detention facility.',
    },
  }
    ...
    map.on('load', () => {
      ...
      const loadHexesForCurrentView = async () => {
        ...
        for (const hexIndex of hexIndexes) {
          ...
          const isOthers = !isOwned && globalOwnedSet.has(hexIndex)
